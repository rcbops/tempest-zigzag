...
def file_test_xml_setup_failure(tmpdir_factory, string_test_xml_setup_failure):

    filename = tmpdir_factory.mktemp('data').join('string_test_xml_teardown_failure.xml').strpath

    with open(filename, 'w') as f:
        f.write(string_test_xml_setup_failure)

    return filename


@pytest.fixture(scope='session')
def file_test_xml_setup_multiple_failures(tmpdir_factory, string_test_xml_setup_multiple_failures):

    filename = tmpdir_factory.mktemp('data').join('string_test_xml_teardown_failure.xml').strpath

    with open(filename, 'w') as f:
        f.write(string_test_xml_setup_multiple_failures)

    return filename


@pytest.fixture(scope='session')
def file_test_xml_setup_class_not_in_list(tmpdir_factory, string_test_xml_setup_class_not_in_list):

    filename = tmpdir_factory.mktemp('data').join('string_test_xml_teardown_failure.xml').strpath

    with open(filename, 'w') as f:
        f.write(string_test_xml_setup_class_not_in_list)
...
@pytest.fixture(scope='session')
def string_test_xml_setup_failure():
    """This fixture represents a failure in a teardown class"""

    junit_xml = \
        """
        <testsuite errors="0" failures="1" name="" tests="6" time="1956.921">
            ...
            <testcase classname="" name="setUpClass (tempest.api.compute.admin.test_aggregates.AggregatesAdminTestJSON)" time="0.000">
...
@pytest.fixture(scope='session')
def string_test_xml_setup_multiple_failures():
    """This fixture represents two separate failures
    in two different teardown classes"""

    junit_xml = \
        """
        <testsuite errors="0" failures="2" name="" tests="2" time="1956.921">
            <testcase classname="" name="setUpClass (tempest.api.compute.admin.test_agents.AgentsAdminTestJSON)" time="0.000">
              <failure type="testtools.testresult.real._StringException">Traceback (most recent call last):
              testtools.testresult.real._StringException: Traceback (most recent call last):
                File "/openstack/venvs/tempest-r14.20.0/tempest/lib/common/rest_client.py", line 829, in _error_checker
                  message=message)
...
@pytest.fixture(scope='session')
def string_test_xml_setup_class_not_in_list():
